...
If unsure about a comment, or the reviewer's suggestion seems at odds with what the user has previously instructed, use the notify_user tool to get the user's opinion before proceeding.
...
4. **Fix it** — make the minimal change that addresses the reviewer's feedback. Follow all project conventions and load any relevant domain skills before writing code:
   - FE changes (`jarvis-helpdesk-fe/`) → MUI `sx` prop, Zustand, React Query patterns
   - BE changes (`jarvis-helpdesk-be/`) → Express thin controllers, service layer
   - AI-Services changes (`jarvis-helpdesk-ai-services/`) → NestJS Module/Controller/Service
   - Agentic changes (`jarvis-helpdesk-agentic/`) → FastAPI, Pydantic DTOs, Celery patterns
...
- If a comment is ambiguous, ask the user via notify_user rather than guessing.
