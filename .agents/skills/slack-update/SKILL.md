...
2. **Collect all changes on this branch vs main**
   For each submodule AND the root repo:
   ```bash
...
3. **Fetch ticket info from Jira** (if ticket ID detected)
   - Use the Jira MCP `mcp_jira_jira_get` tool to get ticket title and description
...
3. **Capture evidence** based on user preference:
   - **Screenshots**: Use `browser_screenshot` for key states
...
Use the Slack MCP server to post to the `biz-dev` channel.

- Search for the channel first using the Slack MCP with query `biz-dev`
