...
4. Categorize the changes by domain:
   - **FE**: `jarvis-helpdesk-fe/src/` — React, MUI, Zustand, React Query
   - **BE**: `jarvis-helpdesk-be/src/` — Express, Sequelize, BullMQ
   - **AI-Services**: `jarvis-helpdesk-ai-services/src/` — NestJS, TypeORM
   - **Agentic**: `jarvis-helpdesk-agentic/app/` — FastAPI, Celery, Qdrant, LlamaIndex
   - **Landing**: `jarvis-helpdesk-landing-page/` — Next.js, Tailwind
   - **DevOps**: `jarvis-helpdesk-devops/` — Docker Compose, scripts
   - **Tests**: test files across any repo

## Step 3: Load context

1. **Load the relevant project skills** based on which domains are touched:
   - FE changes → MUI `sx` preferred, Zustand with persist, React Query for server state (see `jarvis-helpdesk-fe/.agents/skills/conventions/`)
   - BE changes → component-based architecture, thin controllers, logic in `*.service.js` (see `jarvis-helpdesk-be/.agents/skills/conventions/`)
   - AI-Services changes → NestJS Module/Controller/Service pattern (see `jarvis-helpdesk-ai-services/.agents/skills/conventions/`)
   - Agentic changes → LlamaIndex workflows/agents, Celery task patterns (see `jarvis-helpdesk-agentic/.agents/skills/`)
   - Landing changes → Tailwind, Framer Motion, next-intl (see `jarvis-helpdesk-landing-page/.agents/skills/conventions/`)
...
3. **Identify the largest new files** (by line count) — these need the most scrutiny.
