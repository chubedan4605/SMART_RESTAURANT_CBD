...
- **Codebase:** `find`/`grep`/`rg` for scouting relevant files
- **Frontend:** `browser-testing` skill for visual verification (screenshots, console, network)
- **Complex problems:** `sequential-thinking` skill when stuck or decomposing multi-step issues
